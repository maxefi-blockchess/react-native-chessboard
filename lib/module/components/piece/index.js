...
const Piece = /*#__PURE__*/React.memo( /*#__PURE__*/React.forwardRef((_ref, ref) => {
  ...
  const handleOnBegin = useCallback(() => {
    ...
    const previousTappedSquare = selectedSquare.value; // console.log({ previousTappedSquare });

    const move = previousTappedSquare && validateMove(previousTappedSquare, currentSquare); // console.log({ move });
    ...
    console.log({
      gestureEnabledValue: gestureEnabled.value
    });
    ...
    console.log('3');
    onStartTap(square);
  }, [borderColor, gestureEnabled.value, globalMoveTo, onStartTap, scale, selectedSquare.value, square, toPosition, translateX.value, translateY.value, validateMove]);
